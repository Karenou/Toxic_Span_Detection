...
class NERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # roberta use the subword-level label
        else:
            encoded_labels = np.ones(len(encoding["offset_mapping"]), dtype=int) * -100
            i = 0
            for idx, mapping in enumerate(encoding["offset_mapping"]):
                try:
                    if (mapping[0] == 1 or idx == 1) and mapping[1] != 0:
                        encoded_labels[idx] = labels[i]
                        if mapping[1] == 1 and len(sentence[i]) != 1:
                            continue
                        i += 1
                    elif mapping[0] > 1:
    ...
    def collate_fn(self, batch):
        ...
        return [batch_data, batch_flair_feat, batch_attention_masks, batch_labels, sentences, origional_sentence,
                origional_label, length, offset]
