...
def train_epoch(device, train_loader, model, optimizer, scheduler, pooling, epoch):
    ...
    for _, batch_samples in enumerate(tqdm(train_loader)):
        batch_data, batch_flair_input, batch_ft_input, batch_masks, batch_labels, sentence, _, _, length, offset = batch_samples
...
def eval_epoch(device, data_loader, model, pooling, epoch=1, mode="dev", save_path=None):
    ...
    for _, batch_samples in enumerate(tqdm(data_loader)):
        ...
        batch_data, batch_flair_input, batch_ft_input, batch_masks, batch_labels, sentences, origional_sentences, origional_labels, length, offset = batch_samples
        # shift tensors to GPU if available
        batch_data = batch_data.to(device)
        batch_masks = batch_masks.to(device)
        batch_labels = batch_labels.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lstm", type=bool, default=False, help="whether add lstm in classifier")
    parser.add_argument("--bert_avg", type=bool, default=False, help="whether concat 3 layers of bert as input feature")
    ...
    training_set = get_dataset("%s/tsd_train.csv" % args.base_path, split="train", flair_model_path=args.flair_model, fast_text_path=args.fast_text_path)
    trial_set = get_dataset("%s/tsd_trial.csv" % args.base_path, split="trial", flair_model_path=args.flair_model, fast_text_path=args.fast_text_path)
    test_set = get_dataset("%s/tsd_test.csv" % args.base_path, split="test", flair_model_path=args.flair_model, fast_text_path=args.fast_text_path)
