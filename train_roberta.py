...
def eval_epoch(device, data_loader, model, epoch=1, mode="dev", save_path=None):
    """
    ...
    for _, batch_samples in enumerate(tqdm(data_loader)):
        ...
        for i in range(len(length)):
            ...
            pred_labels.append(get_roberta_char_level_label(data_tobe_convert))
...
def train_model(device, train_loader, dev_loader, test_loader, model, optimizer, scheduler, epoch_num):
    """
...
if __name__ == "__main__":
    ...
    model = RobertaNER.from_pretrained("roberta-base")
    ...
    train_model(device, train_loader, trial_loader, test_loader, model, optimizer, scheduler, args.early_stopping)

    print("--------Start Testing!--------")
    eval_epoch(device, test_loader, model, mode="test", save_path=args.save_pred_path)
